...
function addKeyAndValue(arr, key, value) {
    ...
    // return object: {key: 'value' , arr[]} so you want to push the key and value into each object.
...
function partition(arr, callback) {
    //check if each element passes the callback function
    //if true, add to arr1. if false, add to arr2.
    return arr.reduce(function (accum, next) {
        if (callback(next)) {
            accum[0].push(next);
        } else {
